...
\begin{formalcomment}
  In the Sail model, the selection of the rounding mode for an
  instruction is performed by \sailfname{select\_instr\_or\_fcsr\_rm}.
\end{formalcomment}

\begin{formalspec}
  \sailfnselectInstrOrFcsrRm
\end{formalspec}
...
\begin{formalcomment}
  The handling of exception flag accrual in the Sail model is
  performed by \sailfname{accrue\_fflags}.  The specification of some
  instructions use \sailfname{write\_fflags}, which can be taken to be
  equivalent to \sailfname{accrue\_fflags}; the difference between
  them is an artifact of model's interface to the SoftFloat library.
\end{formalcomment}

\begin{formalspec}
  \sailfndirtyFdContext
  \sailfnaccrueFflags
\end{formalspec}
...
\section*{Floating Point in the Sail Specification}
\label{ssec:sail:fp}

The Sail model captures some but not all aspects of the IEEE 754.  The
model specifies for every instruction its encoding and decoding,
interactions with the register file, detection and handling of special
values, and any applicable selection of rounding mode and updates to
the {\tt fcsr} register.  It does not however capture floating-point
arithmetic.  The Sail C emulator performs these computations using the
SoftFloat C library.

The Sail model uses the same instruction specifications for
instructions that are common to both the ``F'' and ``D'' extensions.
As a result, functions like \sailfname{nan\_box} and
\sailfname{nan\_unbox} that appear in this chapter are defined in
Chapter~\ref{sec:double-float} on page~\pageref{nanboxing}.
...
\begin{formalspec}
  \label{lbl:fldst32}
  \sailfnprocessFloadThreeTwo
  \sailfnprocessFloadSixFour
  \sailfclLOADUnderscoreFPexecute
  \sailfnprocessFstore
  \sailfclSTOREUnderscoreFPexecute
\end{formalspec}
\begin{formalcomment}
  See the note on page~\pageref{lbl:ldst32:comment} for an explanation
  of the functions used in specifying instructions that interact with
  memory.
\end{formalcomment}
...
\begin{formalspec}
  \sailfclFUnderscoreBINUnderscoreRMUnderscoreTYPEUnderscoreSexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSexecute
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSCexecute
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSDexecute
\end{formalspec}
...
\begin{formalspec}
  \sailfclFUnderscoreMADDUnderscoreTYPEUnderscoreSexecute
\end{formalspec}
...
\begin{formalspec}
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSAexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSBexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSCexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSDexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSEexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSFexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSGexecute
  \sailfclFUnderscoreUNUnderscoreRMUnderscoreTYPEUnderscoreSHexecute
\end{formalspec}
...
\begin{formalspec}
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSexecute
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSAexecute
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSBexecute
\end{formalspec}
...
\begin{formalspec}
  \sailfclFUnderscoreUNUnderscoreTYPEUnderscoreSAexecute
  \sailfclFUnderscoreUNUnderscoreTYPEUnderscoreSBexecute
\end{formalspec}
...
\begin{formalspec}
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSEexecute
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSFexecute
  \sailfclFUnderscoreBINUnderscoreTYPEUnderscoreSGexecute
\end{formalspec}
...
\begin{formalspec}
  \sailfclFUnderscoreUNUnderscoreTYPEUnderscoreSexecute
\end{formalspec}
